...
func (s *service) GetList(ctx context.Context, limit, offset int) ([]dto.GetBookRequest, error) {
	books, err := s.st.DB().Book().GetList(ctx, limit, offset)
	if err != nil {
		return books, fmt.Errorf("BookService:GetList err -> %s", err.Error())
	}
